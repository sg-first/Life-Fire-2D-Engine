...
typedef std::function<SCCurrentModulus(unsigned long)> SCFun;//SC系数获取函数使用的函数指针

class SC : public QObject//渐变使用的工具类
{
    Q_OBJECT
public:
    SC(float CurrentModulus,float TargetModulus,long times,Item* item,String signfun,Widget *s)
        :CurrentModulus(CurrentModulus),
          TargetModulus(TargetModulus),
          times(times),item(item),signfun(signfun),s(s){}
    SC(float CurrentModulus,float CurrentModulus2,float TargetModulus,float TargetModulus2,long times,Item* item,String signfun,Widget *s)
        :CurrentModulus(CurrentModulus),CurrentModulus2(CurrentModulus2),
        TargetModulus(TargetModulus),TargetModulus2(TargetModulus2),
        times(times),item(item),signfun(signfun),s(s){}
    SC(float CurrentModulus,float CurrentModulus2,float TargetModulus,float TargetModulus2,long times,GraphicsView *gv,String signfun,Widget *s)
        :CurrentModulus(CurrentModulus),CurrentModulus2(CurrentModulus2),
        TargetModulus(TargetModulus),TargetModulus2(TargetModulus2),
        times(times),gv(gv),signfun(signfun),s(s){} //另一个双系数，处理镜头动画
    SC(float CurrentModulus,float CurrentModulus2,float CurrentModulus3,
       float TargetModulus,float TargetModulus2,float TargetModulus3,long times,Item* item,String signfun,Widget *s)
...
protected:
    ...
    Timer *timer;//计时器
    unsigned long temp;//临时变量，在某些特殊情况下用于各个函数之间通信使用
